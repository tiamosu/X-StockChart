...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (dataSet.getGradientColor() != null) {
                final GradientColor gradientColor = dataSet.getGradientColor();
        ...
        for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
            ...
            if (!dataSet.isStacked()) {
                for (int j = 0; j < buffer.buffer.length * mAnimator.getPhaseX(); j += 4) {
                    final float x = (buffer.buffer[j] + buffer.buffer[j + 2]) / 2f;
                ...
                while (index < dataSet.getEntryCount() * mAnimator.getPhaseX()) {
                    ...
                    final float x = (buffer.buffer[bufferIndex] + buffer.buffer[bufferIndex + 2]) / 2f;
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            final BarEntry e = set.getEntryForXValue(high.getX(), high.getY());
            ...
            final boolean isStack = high.getStackIndex() >= 0 && e.isStacked();
            final float y1;
            final float y2;
            if (isStack) {
                if (mChart.isHighlightFullBarEnabled()) {
                    ...
                    final Range range = e.getRanges()[high.getStackIndex()];
                    y1 = range.from;
                    y2 = range.to;
                }
            } else {
