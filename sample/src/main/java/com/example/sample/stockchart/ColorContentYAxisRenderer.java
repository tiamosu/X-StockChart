...
public class ColorContentYAxisRenderer extends YAxisRenderer {
    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        ...
        final int originalColor = mAxisLabelPaint.getColor();
        ...
        if (mYAxis.isValueLineInside()) {
            for (int i = from; i < to; i++) {
                ...
                if (mLabelColorArray != null && mLabelColorArray.length == 3) {
                    if ((!text.endsWith("%") && NumberUtils.String2Double(text) > mClosePrice)
                            || (text.endsWith("%") && NumberUtils.String2Double(text.substring(0, text.length() - 1)) > 0)) {
                        mAxisLabelPaint.setColor(mLabelColorArray[0]);
                    } else if ((!text.endsWith("%") && NumberUtils.String2Double(text) == mClosePrice)
                            || (text.endsWith("%") && NumberUtils.String2Double(text.substring(0, text.length() - 1)) == 0)) {
